...
import { FaDownload, FaYoutube, FaLink, FaCheckCircle, FaExclamationCircle, FaGoogle } from 'react-icons/fa';

export default function CourseMediaModal({ isOpen, onClose, courseId, mediaItems }) {
  const [selectedItems, setSelectedItems] = useState([]);
  const [isDownloading, setIsDownloading] = useState(false);
  const [isUploading, setIsUploading] = useState(false);
  const [uploadType, setUploadType] = useState('youtube'); // 'youtube' or 'drive'
  ...
  const uploadMedia = async () => {
    ...
    try {
      ...
      const platform = uploadType === 'youtube' ? 'YouTube' : 'Google Drive';
      toast.success(`Đã tải lên ${platform} ${result.success} tài liệu thành công`);
    } catch (error) {
      console.error(`Lỗi khi tải lên ${uploadType === 'youtube' ? 'YouTube' : 'Google Drive'}:`, error);
      toast.error(`Lỗi: ${error.message}`);
    } finally {
        ...
        <div className="flex justify-between mb-2">
          ...
          <div className="flex gap-2">
            ...
            <Tabs 
              ...
              onSelectionChange={setUploadType}
            >
              <Tab key="youtube" title="YouTube" />
              <Tab key="drive" title="Google Drive" />
            </Tabs>
            
            <Button 
              size="sm" 
              color={uploadType === 'youtube' ? 'danger' : 'success'} 
              startContent={uploadType === 'youtube' ? <FaYoutube /> : <FaGoogle />}
              isLoading={isUploading}
              onPress={uploadMedia}
            >
              Tải lên {uploadType === 'youtube' ? 'YouTube' : 'Drive'}
            </Button>
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-2">
                    {item.success ? (
                      <>
                        ...
                        {item.driveUrl && (
                          <Tooltip content="Mở liên kết Google Drive">
                            <Button 
                              as="a" 
                              href={item.driveUrl} 
                              target="_blank" 
        ...
        {(onClose) => (
          <>
            ...
            <ModalBody>
              ...
              {renderResultTable(uploadResults, `Kết quả tải lên ${uploadType === 'youtube' ? 'YouTube' : 'Google Drive'}`)}
