/**
 * Script để tìm và thay thế các lệnh gọi connectDB() trong các API routes
 * Chạy script này bằng lệnh: node src/scripts/remove-db-connections.js
 */
...
// Thêm regex để kiểm tra xem file có phải là route handler không
const ROUTE_HANDLER_REGEX = /export\s+async\s+function\s+(GET|POST|PUT|DELETE|PATCH)\s*\(\s*request/g;
...
async function processFile(filePath) {
  try {
    ...
    const hasImport = IMPORT_CONNECT_DB_REGEX.test(content);
    
    // Đặt lại regex để tránh lỗi lastIndex
    CONNECT_DB_REGEX.lastIndex = 0;
    ROUTE_HANDLER_REGEX.lastIndex = 0;
    
    // Kiểm tra xem file có gọi connectDB() không
    const hasConnectDB = CONNECT_DB_REGEX.test(content);
    
    // Kiểm tra xem file có phải là route handler không
    const isRouteHandler = ROUTE_HANDLER_REGEX.test(content);
    
    // Nếu file có cả import và gọi connectDB()
    if (hasImport && hasConnectDB) {
      ...
      // Thay thế các lệnh gọi connectDB()
      if (isRouteHandler) {
        // Nếu là route handler, thay thế bằng lệnh gọi dbMiddleware(request)
        newContent = newContent.replace(
          CONNECT_DB_REGEX,
          `await dbMiddleware(request);`
        );
      } else {
        // Nếu không phải route handler, thay thế bằng comment
        newContent = newContent.replace(
          CONNECT_DB_REGEX,
          `// connectDB() đã được thay thế bởi middleware`
        );
      }
...
// Hàm chính
async function main() {
  console.log('🔍 Bắt đầu tìm và thay thế các lệnh gọi connectDB()...');
